...
REMOTE_DB="postgres"
SQL_SCRIPT_PATH="/var/omadmin_reports/create_ppd_atlas_day.sql"
CSV_FILE_PATH="/var/omadmin_reports/ppd_atlas_day.csv"
LOCAL_DIR="/BEYOND_DATA_OLD/Daily_Data/"
LOG_FILE="/BEYOND_DATA_OLD/LOGS/ppd_atlas_day_script.log"
...
BEGIN
    DROP TABLE IF EXISTS ppd_atlas_day;

    CREATE TABLE ppd_atlas_day AS
    SELECT
...
echo "[$(date)] Table ppd_atlas_day created successfully."

# Export the table to a CSV file
echo "[$(date)] Exporting table to CSV file at ${CSV_FILE_PATH}..."
${REMOTE_PSQL} -U postgres -d ${REMOTE_DB} -c "COPY ppd_atlas_day TO '${CSV_FILE_PATH}' WITH CSV HEADER;"
...
if [ "$MULTI_DAY_MODE" = true ]; then
    ...
    OUTPUT_FILE="${OUTPUT_FOLDER}/ppd_atlas_day_${START_DATE}_to_${END_DATE}.csv"
    ...
    echo "[$(date)] PPD CSV file has been successfully saved to ${OUTPUT_FILE}."
else
    ...
    echo "[$(date)] PPD CSV file has been successfully downloaded to ${LOCAL_DIR}."
fi

echo "[$(date)] PPD Script execution completed."
