...
REMOTE_DB="postgres"
SQL_SCRIPT_PATH="/var/omadmin_reports/create_rfnd_atlas_day.sql"
CSV_FILE_PATH="/var/omadmin_reports/rfnd_atlas_day.csv"
LOCAL_DIR="/BEYOND_DATA_OLD/Daily_Data/"
LOG_FILE="/BEYOND_DATA_OLD/LOGS/rfnd_atlas_day_script.log"
...
BEGIN
    DROP TABLE IF EXISTS rfnd_atlas_day;

    CREATE TABLE rfnd_atlas_day AS
    SELECT
        msisdn as tmuserid,
        cpc,
...
echo "[$(date)] Table rfnd_atlas_day created successfully."

# Export the table to a CSV file
echo "[$(date)] Exporting table to CSV file at ${CSV_FILE_PATH}..."
${REMOTE_PSQL} -U postgres -d ${REMOTE_DB} -c "COPY rfnd_atlas_day TO '${CSV_FILE_PATH}' WITH CSV HEADER;"
...
if [ "$MULTI_DAY_MODE" = true ]; then
    ...
    OUTPUT_FILE="${OUTPUT_FOLDER}/rfnd_atlas_day_${START_DATE}_to_${END_DATE}.csv"
    ...
    echo "[$(date)] REFUND CSV file has been successfully saved to ${OUTPUT_FILE}."
else
    ...
    echo "[$(date)] REFUND CSV file has been successfully downloaded to ${LOCAL_DIR}."
fi

echo "[$(date)] REFUND Script execution completed."
