...
12. **Refund Counting:** Always sum `rfnd_cnt` column, never count rows
13. **Parquet Sync:** Regenerate parquet files when source CSVs are updated

---

## 🔄 Recent Critical Fixes (2026-01-17)

### Refund Count Logic Fix
**Issue:** `rfnd_count` was counting rows instead of summing `rfnd_cnt` column.

**Files Modified:**
- `Scripts/05_build_counters.py:compute_daily_cpc_counts()` - Changed to sum `rfnd_cnt`
- `Scripts/utils/counter_utils.py:load_transactions_for_date()` - Ensured `rfnd_cnt` is loaded

**Validation:** Beauty & Health Dec 2025 now shows 7,001 refunds (was 737) ✅

### Parquet Data Synchronization
**Issue:** Parquet files were outdated, missing 436 rows for December 2025.

**Solution:** Re-ran `Scripts/00_convert_historical.py` to sync with `rfnd_atlas.csv`

**Validation:** Parquet data now matches source CSVs exactly ✅
