...
#
# Usage:
#   ./fetch_remote_data.sh <type> [start_date] [end_date]
#
# Parameters:
#   type: act, reno, dct, ppd, cnr, rfnd
...
if [ -z "$TRANS_TYPE" ]; then
    ...
    echo "Types: act, reno, dct, ppd, cnr, rfnd"
    exit 1
fi

# Validate Transaction Type
case "$TRANS_TYPE" in
    act|reno|dct|ppd|cnr|rfnd) ;;
    *) echo "Error: Invalid transaction type '$TRANS_TYPE'. Must be one of: act, reno, dct, ppd, cnr, rfnd"; exit 1 ;;
esac
...
LOG_FILE="${LOG_DIR}/fetch_${TRANS_TYPE}_${START_DATE}.log"
exec > >(tee -a ${LOG_FILE}) 2>&1

echo "[$(date)] Starting fetch for TYPE: $TRANS_TYPE"

# ------------------------------------------------------------------------------
# SQL Generation
# ------------------------------------------------------------------------------

SQL_SCRIPT_PATH="/var/omadmin_reports/create_${TRANS_TYPE}_atlas_day.sql"
CSV_FILE_PATH="/var/omadmin_reports/${TRANS_TYPE}_atlas_day.csv"
...
BEGIN
    DROP TABLE IF EXISTS ${TRANS_TYPE}_atlas_day;
    CREATE TABLE ${TRANS_TYPE}_atlas_day AS
    ${SQL_QUERY}
END \$\$;
SQL
...
${REMOTE_PSQL} -U postgres -d ${REMOTE_DB} -c "COPY ${TRANS_TYPE}_atlas_day TO '${CSV_FILE_PATH}' WITH CSV HEADER;"
EOF
...
if [ $? -eq 0 ]; then
    echo "[$(date)] SUCCESS: ${TRANS_TYPE} data fetched."
else
